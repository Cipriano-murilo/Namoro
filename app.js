...
function initFotos() {
  ...
  btnEnv.addEventListener('click', async () => {
    ...
    try {
      const data = await fetch(`${API_BASE}/fotos.php`, {
        method: 'POST',
        credentials: 'include',
        body: fd,
      }).then(r => r.json());
...
async function carregarFotos() {
  try {
    const data = await api('fotos.php');
...
async function deletarFoto(id) {
  ...
  try {
    await api(`fotos.php?id=${id}`, { method: 'DELETE', headers: {} });
...
async function enviarSentimento() {
  ...
  try {
    const data = await api('sentimentos.php', {
      method: 'POST',
...
async function carregarSentimentos() {
  try {
    const data = await api('sentimentos.php?limit=50');
...
async function deletarSentimento(id) {
  ...
  try {
    await api(`sentimentos.php?id=${id}`, { method: 'DELETE', headers: {} });
